...
namespace EP.IdentityIsolation.Infra.CrossCutting.Identity.Configuration
{
    ...
    {
        public ApplicationUserManager2(IUserStore<ApplicationUser> store) 
            : base(store)
        {
        ...
        public static ApplicationUserManager2 Create(IdentityFactoryOptions<ApplicationUserManager2> options, IOwinContext context)
        {
            var appcontext = context.Get<ApplicationDbContext2>();
            var usuarioManager = new ApplicationUserManager2(new UserStore<ApplicationUser>(appcontext));
